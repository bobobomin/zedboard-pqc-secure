...
module mlkem_poly_tomsg_controller(
    ...
    /* Compress_1(x)=round(2x/q) mod 2 is 1 exactly on 833<=x<=2496 for canonical
       x in [0,3328], so two comparisons replace the 16x32 constant multiply. */
    ...
    always_ff @(posedge clk_i or negedge rst_ni)begin
        if(!rst_ni)begin state<=IDLE;index<=0;slot<=0;message_o<=0;end
            ...
            CAPTURE:begin message_o[index]<=message_bit;if(index==255)state<=DONE;
