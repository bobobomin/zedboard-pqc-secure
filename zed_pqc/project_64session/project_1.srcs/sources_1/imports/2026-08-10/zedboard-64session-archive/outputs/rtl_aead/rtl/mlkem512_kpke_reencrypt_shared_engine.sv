...
module mlkem512_kpke_reencrypt_shared_engine(
    ...
    logic bs,bd,brdy,bwe;logic[1:0]bcmd;logic[3:0]bsa,bsb,bsd;logic[11:0]bpa;logic[15:0]bpd;
    ...
    mlkem_poly_bridge_controller bridge(clk_i,rst_ni,bs,bcmd,bsa,bsb,bsd,,brdy,bd,bwe,bpa,bpd,poly_rdata_i);
    ...
    always_ff @(posedge clk_i or negedge rst_ni)begin
      if(!rst_ni)begin state<=IDLE;hash_fail_q<=0;end else begin
        ...
        NEP_S:state<=NEP_W;NEP_W:if(hash_done_i)state<=NT0_S;NT0_S:state<=NT0_W;NT0_W:if(brdy)state<=NT1_S;
        ...
        CB1_S:state<=CB1_W;CB1_W:if(cd)state<=VP0_S;VP0_S:state<=VP0_W;VP0_W:if(brdy)state<=VP1_S;
