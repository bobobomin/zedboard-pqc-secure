...
/* Deterministic ML-KEM-512 K-PKE.Encrypt used by Decaps re-encryption. */
module mlkem512_kpke_reencrypt_engine(
    ...
    logic[6:0]state;
    logic ns,nd,nwe,eta3;logic[7:0]nonce;logic[3:0]nslot;logic[11:0]npa;logic[15:0]npd;
    logic ms,md,mwe,merr;logic[7:0]mx,my;logic[3:0]mslot;logic[11:0]mpa;logic[15:0]mpd;
    logic fs,fd,fwe;logic[11:0]fpa;logic[15:0]fpd;
    ...
    always_comb begin
        fs=state==FM_S;
        ns=state==NS0_S||state==NS1_S||state==NE0_S||state==NE1_S||state==NEP_S;
        eta3=state==NS0_S||state==NS0_W||state==NS1_S||state==NS1_W;
        nonce=0;nslot=2;
        if(in_pair(state,NS1_S,NS1_W))begin nonce=1;nslot=3;end
        else if(in_pair(state,NE0_S,NE0_W))begin nonce=2;nslot=4;end
        else if(in_pair(state,NE1_S,NE1_W))begin nonce=3;nslot=5;end
        else if(in_pair(state,NEP_S,NEP_W))begin nonce=4;nslot=6;end
        ms=state==M00_S||state==M01_S||state==M10_S||state==M11_S;
        mx=0;my=0;mslot=8;
        if(in_pair(state,M01_S,M01_W))my=1;
        else if(in_pair(state,M10_S,M10_W))mx=1;
        else if(in_pair(state,M11_S,M11_W))begin mx=1;my=1;end
        bs=state==NT0_S||state==NT1_S||state==B00_S||state==B01_S||
           state==IB0_S||state==B10_S||state==B11_S||state==IB1_S||
           state==VP0_S||state==VP1_S||state==VI_S;
        ...
        as=state==AB0_S||state==EB0_S||state==AB1_S||state==EB1_S||
           state==VA_S||state==VE_S||state==VM_S;sub=0;asa=9;asb=10;asd=11;
        ...
        else if(state>=NS0_S&&state<=NEP_W)begin poly_we_o=nwe;poly_addr_o=npa;poly_wdata_o=npd;end
        else if(state==M00_S||state==M00_W||state==M01_S||state==M01_W||
          state==M10_S||state==M10_W||state==M11_S||state==M11_W)begin
            poly_we_o=mwe;poly_addr_o=mpa;poly_wdata_o=mpd;end
        else if(state==NT0_S||state==NT0_W||state==NT1_S||state==NT1_W||
          state==B00_S||state==B00_W||state==B01_S||state==B01_W||
          state==IB0_S||state==IB0_W||state==B10_S||state==B10_W||
          state==B11_S||state==B11_W||state==IB1_S||state==IB1_W||
          state==VP0_S||state==VP0_W||state==VP1_S||state==VP1_W||state==VI_S||state==VI_W)begin
            poly_we_o=bwe;poly_addr_o=bpa;poly_wdata_o=bpd;end
        else if(state==AB0_S||state==AB0_W||state==EB0_S||state==EB0_W||
          state==AB1_S||state==AB1_W||state==EB1_S||state==EB1_W||
          state==VA_S||state==VA_W||state==VE_S||state==VE_W||state==VM_S||state==VM_W)begin
            poly_we_o=awe;poly_addr_o=apa;poly_wdata_o=apd;end
        else begin poly_we_o=0;poly_addr_o=cpa;poly_wdata_o=0;end
        busy_o=state!=IDLE;done_o=state==DONE;mismatch_o=cmis|merr;
    end
    always_ff @(posedge clk_i or negedge rst_ni)begin
        if(!rst_ni)state<=IDLE;else case(state)
          IDLE:if(start_i)state<=FM_S;FM_S:state<=FM_W;FM_W:if(fd)state<=NS0_S;
          NS0_S:state<=NS0_W;NS0_W:if(nd)state<=NS1_S;NS1_S:state<=NS1_W;NS1_W:if(nd)state<=NE0_S;
          NE0_S:state<=NE0_W;NE0_W:if(nd)state<=NE1_S;NE1_S:state<=NE1_W;NE1_W:if(nd)state<=NEP_S;
          NEP_S:state<=NEP_W;NEP_W:if(nd)state<=NT0_S;NT0_S:state<=NT0_W;NT0_W:if(bd)state<=NT1_S;
          NT1_S:state<=NT1_W;NT1_W:if(bd)state<=M00_S;M00_S:state<=M00_W;M00_W:if(md)state<=B00_S;
          B00_S:state<=B00_W;B00_W:if(bd)state<=M01_S;M01_S:state<=M01_W;M01_W:if(md)state<=B01_S;
          ...
          CB0_S:state<=CB0_W;CB0_W:if(cd)state<=M10_S;M10_S:state<=M10_W;M10_W:if(md)state<=B10_S;
          B10_S:state<=B10_W;B10_W:if(bd)state<=M11_S;M11_S:state<=M11_W;M11_W:if(md)state<=B11_S;
