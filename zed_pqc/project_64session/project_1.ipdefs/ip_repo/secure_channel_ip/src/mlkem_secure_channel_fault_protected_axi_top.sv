...
module mlkem_secure_channel_fault_protected_axi_top #(
    ...
    typedef enum logic[3:0]{IDLE,D_START,D_WAIT,T_START,T_WAIT,K_START,K_WAIT,
        C_START,C_WAIT,REPORT}st_t;st_t state,prev_state;
    ...
    always_comb begin
      ...
      if(state==T_START)begin hs_req=!(fault_detected_o||(ss_a!=~ss_b));hcmd_req=5;hd0=0;hd1=0;
        hx=0;hy=0;hnonce=0;heta=0;hslot=0;end
      else if(state==K_START)begin hs_req=!(fault_detected_o||(transcript_a!=~transcript_b));
      ...
      install_req=(state==C_START)&&!fault_detected_o&&(material_a==~material_b)&&(hash_index==14);
    ...
    always_ff @(posedge s_axi_aclk or negedge s_axi_aresetn)begin
      if(!s_axi_aresetn)begin state<=IDLE;prev_state<=IDLE;ss_a<=0;ss_b<=~256'd0;
        ...
        fault_detected_o<=0;fault_code_o<=0;hash_index<=0;stage_timer<=0;outstanding<=0;end
      else begin
        prev_state<=state;
        ...
        case(state)
          ...
          D_WAIT:if(core_done)begin ss_a<=ss^{255'd0,fault_inject_i[1]};ss_b<=~ss;
            ...
            state<=core_fail?REPORT:T_START;end
          T_START:begin if(fault_detected_o||(ss_a!=~ss_b))begin fault_detected_o<=1;
          ...
          T_WAIT:if(hdone_ctl)begin transcript_a<=hdigest[255:0]^{255'd0,fault_inject_i[2]};
            transcript_b<=~hdigest[255:0];state<=K_START;end
          K_START:begin if(fault_detected_o||(transcript_a!=~transcript_b))begin fault_detected_o<=1;
          ...
          K_WAIT:if(hdone_ctl)begin material_a<=hdigest^{575'd0,fault_inject_i[3]};
            material_b<=~hdigest;state<=C_START;end
          C_START:begin if(fault_detected_o||(material_a!=~material_b)||(hash_index!=14))begin
