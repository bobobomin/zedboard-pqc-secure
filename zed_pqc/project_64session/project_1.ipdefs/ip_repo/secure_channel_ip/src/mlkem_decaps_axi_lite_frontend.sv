...
module mlkem_decaps_axi_lite_frontend #(
    ...
    localparam[7:0]REG_VERSION=0,REG_CONTROL=4,REG_STATUS=8,REG_SLOT=12,
        ...
        REG_CYCLES=44;
    logic aw_hold,w_hold;logic[7:0]awaddr_q;logic[31:0]wdata_q;
    ...
    logic cycle_running;logic[31:0]cycle_counter,cycle_result;
    assign s_axi_awready=!aw_hold&&!s_axi_bvalid;
    assign s_axi_wready=!w_hold&&!s_axi_bvalid;assign s_axi_bresp=0;
    assign s_axi_arready=!s_axi_rvalid&&!read_mem_pending;assign s_axi_rresp=0;
    ...
    always_ff @(posedge s_axi_aclk)begin
        if(!s_axi_aresetn)begin aw_hold<=0;w_hold<=0;s_axi_bvalid<=0;s_axi_rvalid<=0;
            s_axi_rdata<=0;region<=0;mem_addr<=0;mem_access_addr<=0;mem_valid<=0;mem_we<=0;
            read_mem_pending<=0;decap_start_o<=0;decap_slot_o<=0;
            decap_session_id_o<=0;done_latched<=0;fail_latched<=0;
            cycle_running<=0;cycle_counter<=0;cycle_result<=0;end
        else begin
            mem_valid<=0;mem_we<=0;decap_start_o<=0;
            if(decap_start_o)begin
                cycle_running<=1;cycle_counter<=0;
            end else if(cycle_running)begin
                if(decap_done_i)begin
                    cycle_running<=0;cycle_result<=cycle_counter+1;
            ...
            if(s_axi_wvalid&&s_axi_wready)begin wdata_q<=s_axi_wdata;w_hold<=1;end
            if(aw_hold&&w_hold&&!s_axi_bvalid)begin
                case(awaddr_q)
                    ...
                    REG_MEM_DATA:begin mem_access_addr<=mem_addr;mem_valid<=1;
                        mem_we<=1;mem_addr<=mem_addr+1;end
                    default:begin end
                endcase
            ...
            if(s_axi_arvalid&&s_axi_arready)begin
                case(s_axi_araddr)
                    ...
                    REG_CYCLES:begin s_axi_rdata<=cycle_result;s_axi_rvalid<=1;end
                    REG_MEM_DATA:begin mem_access_addr<=mem_addr;mem_valid<=1;
            ...
            if(read_mem_pending&&mem_ready)begin s_axi_rdata<=mem_rdata;
                s_axi_rvalid<=1;read_mem_pending<=0;mem_addr<=mem_addr+1;end
