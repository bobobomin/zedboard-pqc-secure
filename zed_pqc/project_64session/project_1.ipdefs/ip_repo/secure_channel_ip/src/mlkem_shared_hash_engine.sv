...
module mlkem_shared_hash_engine(
    ...
    typedef enum logic[3:0]{IDLE,HSTART,FEED,FINAL,OUT,CHECK,W0,W1,NWRITE,DRAIN,DONE}st_t;st_t state;
    ...
    always_comb begin
        ...
        /* The word address comes from fetch_index, which runs ahead of in_index.
           Every memory region starts on a word boundary of in_index (0, 32, 800),
           so the byte select below is always in_index[1:0]. */
        sk_addr_o=0;ct_addr_o=0;fetch_word=sk_rdata_i;
        if(cmd==C_HPK)sk_addr_o=(768+fetch_index)>>2;
        else if(cmd==C_J)begin ct_addr_o=(fetch_index-32)>>2;fetch_word=ct_rdata_i;end
        else if(cmd==C_TRANSCRIPT&&fetch_index<800)sk_addr_o=(768+fetch_index)>>2;
        else if(cmd==C_TRANSCRIPT&&fetch_index<1568)begin ct_addr_o=(fetch_index-800)>>2;
            fetch_word=ct_rdata_i;end
        /* Register operands are selected combinationally too, so they also cost
           one cycle per byte.  The 5-bit slice keeps the 256-bit operand muxes
           out of the 11-bit index arithmetic. */
        if(memory_input(cmd,in_index))case(in_index[1:0])
            0:inbyte=word_cur[7:0];1:inbyte=word_cur[15:8];
            2:inbyte=word_cur[23:16];default:inbyte=word_cur[31:24];endcase
        else case(cmd)
            C_G:inbyte=in_index<32?d0[8*in_index[4:0]+:8]:d1[8*in_index[4:0]+:8];
            C_J:inbyte=d0[8*in_index[4:0]+:8];
            C_MATRIX:inbyte=in_index<32?d0[8*in_index[4:0]+:8]:(in_index==32?x:y);
            C_NOISE:inbyte=in_index<32?d0[8*in_index[4:0]+:8]:nonce;
        ...
        fetch_en=state==FEED&&!fetch_wait&&memory_input(cmd,fetch_index)&&
            fetch_index<input_len&&(!cur_val||!nxt_val||word_shift);
    ...
    always_ff @(posedge clk_i or negedge rst_ni)begin
        if(!rst_ni)begin state<=IDLE;cmd<=0;d0<=0;d1<=0;sid<=0;x<=0;y<=0;nonce<=0;
          ...
          HSTART:state<=FEED;
          FEED:begin
              fetch_wait<=fetch_en;if(fetch_en)fetch_index<=fetch_index+11'd4;
              if(word_shift)begin word_cur<=word_nxt;cur_val<=nxt_val;nxt_val<=0;end
              if(fetch_en)begin
